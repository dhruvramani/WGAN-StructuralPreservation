...
def train_aesthecity():
    ...
    for epoch in range(sepoch, args.epochs):
        ...
        for i in range(step, le):
            ...
            real_img, fake_img, fake_img2 = real_img.type(torch.DoubleTensor), fake_img.type(torch.DoubleTensor), fake_img2.type(torch.DoubleTensor)
            imgs = torch.cat((fake_img2, real_img, fake_img))
            labels = torch.cat((label0, label1, label0)).type(torch.FloatTensor)
