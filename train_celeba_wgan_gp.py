...
def train_aesthecity():
    ...
    for epoch in range(sepoch, args.epochs):
        ...
        for i in range(step, le):
            ...
            real_img, fake_img, fake_img2 = real_img.type(torch.FloatTensor).to(device), fake_img.type(torch.FloatTensor).to(device), fake_img2.type(torch.FloatTensor).to(device)
            imgs = torch.cat((fake_img2, real_img, fake_img)).to(device)
            labels = torch.cat((label0, label1, label0)).type(torch.FloatTensor).to(device)
