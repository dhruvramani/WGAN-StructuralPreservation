...
class AsthecitiyClassifier(nn.Module):
    def __init__(self, num_classes=2):
        super(AsthecitiyClassifier, self).__init__()
    ...
    def forward(self, x):
        print(x.shape)
        pad = (int((218 - x.shape[2]) / 2), int((218 - x.shape[2]) / 2), int((178 - x.shape[3]) / 2), int((178 - x.shape[3]) / 2))
