...
class AugumentedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.cat((real_img, fake_img)), torch.cat((torch.Tensor([1, 0]) torch.Tensor([0, 1]))).type(torch.LongTensor) # real_img, torch.tensor([1]).type(torch.LongTensor), fake_img, torch.tensor([0]).type(torch.LongTensor) 
