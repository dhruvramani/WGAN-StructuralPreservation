...
class AugumentedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return real_img, torch.tensor([1]).type(torch.LongTensor), fake_img, torch.tensor([0]).type(torch.LongTensor) #torch.stack((real_img, fake_img)), torch.Tensor([1, 0]).type(torch.LongTensor)
